      ...
      <Card><CardHeader><div className="flex items-center gap-2"><BookCheck className="h-5 w-5 text-blue-600" /><CardTitle>Lançar Notas</CardTitle></div></CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-muted/50 rounded-lg">
            <div><Label>Turma e Matéria</Label><Select value={selectedVinculo} onValueChange={(v: string) => { setSelectedVinculo(v); setSelectedTipo(''); }}><SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent>{vinculos.map((v) => (<SelectItem key={v.id_ptm} value={`${v.id_turma}-${v.id_materia}`}>{v.nome_turma} - {v.nome_materia}</SelectItem>))}</SelectContent></Select></div>
            <div><Label>Unidade</Label><Select value={selectedUnidade} onValueChange={(v: string) => { setSelectedUnidade(v); setSelectedTipo(''); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="1">1ª Unidade</SelectItem><SelectItem value="2">2ª Unidade</SelectItem><SelectItem value="3">3ª Unidade</SelectItem><SelectItem value="4">4ª Unidade</SelectItem></SelectContent></Select></div>
            <div><Label>Tipo de Avaliação</Label><Select value={selectedTipo} onValueChange={(v: string) => setSelectedTipo(v as TipoAvaliacao)} disabled={!selectedVinculo}><SelectTrigger><SelectValue placeholder={!selectedVinculo ? "Selecione turma/unidade" : "Selecione..."} /></SelectTrigger><SelectContent>{tiposDisponiveis.map((tipo) => (<SelectItem key={tipo} value={tipo}>{tiposLabel[tipo]}</SelectItem>))}</SelectContent></Select></div>
          </div>
