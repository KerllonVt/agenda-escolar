...
export function Atividades({ onBack }: AtividadesProps) {
  ...
  const fetchAtividades = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_URL}/atividades`, { headers: { 'Authorization': `Bearer ${token}` } });
      if (!response.ok) throw new Error('Falha ao buscar atividades.');
      const data = await response.json(); setAtividades(data);
