...
import { Users, Plus, Pencil, Trash2, GraduationCap, Loader2, Search } from 'lucide-react';
...
export default function GerenciarTurmasSeries() {
  ...
  const debouncedSearchTurma = useDebounce(searchTurma, 300);
  const debouncedSearchAluno = useDebounce(searchAluno, 300);
  ...
  // --- API: Buscar Turmas (com pesquisa) ---
  const fetchTurmas = async (searchTerm = '') => {
    setIsLoading(true);
    try {
      const url = searchTerm ? `${API_URL}/turmas?search=${searchTerm}` : `${API_URL}/turmas`;
  ...
  // --- API: Buscar Alunos (com pesquisa) ---
  const fetchAlunos = async (searchTerm = '') => {
    try {
      let url = `${API_URL}/users?tipo=aluno`;
      if (searchTerm) {
  ...
  useEffect(() => {
    if (token) fetchTurmas(debouncedSearchTurma);
  }, [token, debouncedSearchTurma]);
  
  useEffect(() => {
    if (token) fetchAlunos(debouncedSearchAluno);
  }, [token, debouncedSearchAluno]);
  ...
  const handleAlocarAluno = async (alunoId: number, turmaId: number | null) => {
    try {
      ...
      setAlunos(alunos.map(a => 
        a.id_usuario === alunoId ? { ...a, id_turma: turmaId ?? undefined } : a
      ));
      fetchTurmas(); 
    } catch (error: any) {
      toast.error(error.message);
    ...
    <>
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <div className="relative w-full max-w-sm mt-4">
              ...
              <Input 
                placeholder="Pesquisar turma ou série..." 
                className="pl-9"
                value={searchTurma}
          ...
          <CardContent>
            {isLoading ? (
              <div className="flex justify-center items-center py-10"><Loader2 className="w-8 h-8 animate-spin text-muted-foreground" /></div>
            ) : (
              <Table>
                <TableHeader><TableRow><TableHead>Turma</TableHead><TableHead>Série</TableHead><TableHead>Ano Letivo</TableHead><TableHead>Turno</TableHead><TableHead>Alunos</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader>
                <TableBody>
                  {turmas.map((turma) => (
                    <TableRow key={turma.id_turma}>
                      ...
                      <TableCell><Badge>{turma.total_alunos} alunos</Badge></TableCell>
                      <TableCell className="text-right"><div className="flex gap-2 justify-end">
                        <Button variant="ghost" size="sm" onClick={() => abrirDialogEditar(turma)}>
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleExcluirTurma(turma.id_turma)}>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              <TableHeader><TableRow><TableHead>Nome do Aluno</TableHead><TableHead>Email</TableHead><TableHead>Turma Atual</TableHead><TableHead>Alocar para Turma</TableHead></TableRow></TableHeader>
              <TableBody>
                {alunos.map((aluno) => (
                  <TableRow key={aluno.id_usuario}>
                    ...
                    <TableCell><Badge variant={aluno.id_turma ? "default" : "secondary"}>{getTurmaNome(aluno.id_turma)}</Badge></TableCell>
                    <TableCell>
                      <Select
                        value={aluno.id_turma?.toString() || 'sem-turma'}
                        onValueChange={(value: string) => handleAlocarAluno(aluno.id_usuario, value === 'sem-turma' ? null : Number(value))}
                      >
      ...
      {/* Modal de Criar/Editar Turma (agora está no final) */}
      <Dialog open={dialogAberto} onOpenChange={setDialogAberto}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div><Label htmlFor="nome_turma">Nome da Turma *</Label><Input id="nome_turma" placeholder="Ex: 9° A, 1° B EM" value={novaTurma.nome_turma} onChange={(e) => setNovaTurma({ ...novaTurma, nome_turma: e.target.value })} /></div>
            <div><Label htmlFor="serie">Série *</Label><Input id="serie" placeholder="Ex: 9° Ano, 1° Ano Ensino Médio" value={novaTurma.serie} onChange={(e) => setNovaTurma({ ...novaTurma, serie: e.target.value })} /></div>
            <div><Label htmlFor="ano">Ano Letivo</Label><Input id="ano" placeholder="2025" value={novaTurma.ano} onChange={(e) => setNovaTurma({ ...novaTurma, ano: e.target.value })} /></div>
            <div><Label htmlFor="turno">Turno</Label><Select value={novaTurma.turno} onValueChange={(value: string) => setNovaTurma({ ...novaTurma, turno: value as any })}><SelectTrigger id="turno"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Matutino">Matutino</SelectItem><SelectItem value="Vespertino">Vespertino</SelectItem><SelectItem value="Noturno">Noturno</SelectItem></SelectContent></Select></div>
            <div className="flex gap-2 justify-end"><Button variant="outline" onClick={() => setDialogAberto(false)}>Cancelar</Button><Button onClick={handleSalvarTurma}>{turmaEditando ? 'Salvar' : 'Criar Turma'}</Button></div>
          </div>
        </DialogContent>
      </Dialog>
    </>
