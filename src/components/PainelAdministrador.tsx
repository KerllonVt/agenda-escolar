...
export function PainelAdministrador({ onBack }: PainelAdministradorProps) {
  ...
  const handleCadastrarUsuario = async () => {
    ...
    try {
      ...
      setNovoUsuario({ nome_completo: '', email: '', senha: '', tipo_usuario: 'aluno' });
      fetchUsers(); 
    } catch (error: any) {
      console.error(error);
      toast.error(error.message);
  ...
  const handleCadastrarMateria = async () => {
    ...
    try {
      ...
      setNovaMateria({ nome_materia: '' }); // Limpa o campo
      fetchMaterias(); // Atualiza a lista
    } catch (error: any) {
      toast.error(error.message);
    } finally {
    ...
    if (errorUsers) {
      return (
        <div className="flex justify-center items-center py-10 text-red-600">
          ...
          <p className="ml-2">Erro ao carregar usuários: {errorUsers}</p>
        </div>
      );
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-primary">Painel do Administrador</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <Card className="border-2 border-orange-200 bg-orange-50">
                ...
                <CardTitle className="text-3xl">{materias.length}</CardTitle>
                </CardHeader>
            </Card>
        </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <CardHeader>
              <TabsList className="grid w-full grid-cols-6">
                ...
                <TabsTrigger value="turmas" disabled>
                  <School className="w-4 h-4 mr-2" />
                  Turmas
                </TabsTrigger>
                <TabsTrigger value="turmas-series">
                  <GraduationCap className="w-4 h-4 mr-2" />
                  Gestão Turmas
                </TabsTrigger>
                <TabsTrigger value="professores-turmas">
            ...
            <CardContent>
              ...
              <TabsContent value="usuarios" className="space-y-6">
                <Card className="bg-muted">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button onClick={handleCadastrarUsuario} className="w-full" disabled={isCreatingUser}>
                      {isCreatingUser ? (
              ...
              <TabsContent value="professores-turmas">
              ...
              <TabsContent value="materias" className="space-y-6">
                <Card className="bg-muted">
                  <CardHeader>
                    <CardTitle className="text-lg">Cadastrar Nova Matéria</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        id="nome-materia"
                        value={novaMateria.nome_materia}
                        onChange={(e) => setNovaMateria({ ...novaMateria, nome_materia: e.target.value })}
                        placeholder="Ex: Matemática"
                      />
                    </div>
                    <Button onClick={handleCadastrarMateria} className="w-full" disabled={isCreatingMateria}>
                      {isCreatingMateria ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <BookOpen className="w-4 h-4 mr-2" />}
                      Cadastrar Matéria
                    </Button>
                  </CardContent>
                </Card>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {materias.map((materia) => (
                        <Card key={materia.id_materia}>
                          <CardHeader>
                            <div className="flex items-center justify-between">
                              <CardTitle>{materia.nome_materia}</CardTitle>
